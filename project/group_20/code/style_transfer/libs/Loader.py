...
class Dataset(data.Dataset):
    def __init__(self, dataPath, loadSize, fineSize, test=False, video=False):
        ...
        self.tmp_image_list = [x for x in os.listdir(
            dataPath) if is_image_file(x)]
        self.image_list = []
        if(video):
            for j in range(len(self.tmp_image_list)):
                self.image_list.append(
                    dataPath + "/frame{}.jpg".format(str(j)))
        else:
            self.image_list = self.tmp_image_list
        if not test:
