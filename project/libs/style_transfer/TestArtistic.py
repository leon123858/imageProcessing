...
parser.add_argument("--stylePath", default="../../styleImage/",
                    help='path to style image')
parser.add_argument("--contentPath", default="../../sampleData/",
                    help='path to frames')
parser.add_argument("--outf", default="../../cache/",
...
opt.matrixPath = opt.matrixPath.replace("r31", opt.layer)
opt.matrixPath = opt.matrixPath.replace("r41", opt.layer)
...
content_dataset = Dataset(
    opt.contentPath, opt.loadSize, opt.fineSize, test=True)
content_loader = torch.utils.data.DataLoader(dataset=content_dataset,
                                             batch_size=opt.batchSize,
                                             shuffle=False,
                                             num_workers=1 if opt.cuda == True else 0)
...
for ci, (content, contentName) in enumerate(content_loader):
    ...
    for sj, (style, styleName) in enumerate(style_loader):
        ...
        vutils.save_image(transfer, '%s/%s_%s.png' % (opt.outf, contentName,
                          styleName), normalize=True, scale_each=True, nrow=opt.batchSize)
        print('Transferred image saved at %s%s_%s.png' %
              (opt.outf, contentName, styleName))
